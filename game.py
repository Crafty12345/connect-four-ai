...
parser.add_argument("--output-stats", action=BooleanOptionalAction, type=bool,
...
parser.add_argument("--count-states", action=BooleanOptionalAction, type=bool, required=False, default=False,
                    help="""Whether or not to count
                    the number of unique board states.""")
parser.add_argument("--visualise", action=BooleanOptionalAction, type=bool, required=False, default=True,
...
SCREEN_SIZE = (600, 600)
BOARD_SIZE = Rect(0, 100, SCREEN_SIZE[0], SCREEN_SIZE[1])


if args.visualise == True:
    init()
    pointer_yellow_img = image.load("yellow_triangle.png")
    pointer_red_img = image.load("red_triangle.png")

    end_game_font = font.Font("freesansbold.ttf", 56)
    score_font = font.Font("freesansbold.ttf", 34)

    display.set_caption("Connect Four Bot")
...
def process_stats(lst_horizontal: list, lst_vertical: list,
    ...
    space_efficiency = sigmoid(
        pos_spaces, a=0.65, c=-3.5) / n_turns[status] if n_turns[status] > 0 else 0
    ...
    if (len(lst_horizontal) > 0 and
        ...
        len(lst_downRight) > 0 and
            len(lst_upRight) > 0):
         stats["horizontal"]["std"] = std(lst_horizontal)
         stats["vertical"]["std"] = std(lst_vertical)
         stats["down_right_diagonal"]["std"] = std(lst_downRight)
         stats["up_right_diagonal"]["std"] = std(lst_upRight)
         ...
         for index in indexes:
             count_summarised[index] = sigmoid(custom_exp(
                 average((count_list[index])), 5, 64.67, 3.67))
             #count_summarised[index] = np.average((count_list[index]))
             stats[index]["std"] = std(count_list[index])
             stats[index]["mean"] = mean(count_list[index])
             stats[index]["median"] = median(count_list[index])
...
running = True
if args.visualise == True and args.mode != "RvR":
    while running:
        """
        Main Loop
        """
        ...
        if args.visualise == True:
            for event in event.get():
                if event.type == KEYDOWN:
                    if not game_ended:
                        if event.key == K_LEFT and pointer.selected != 0:
                            pointer.update_position(pointer.selected-1)
                        if event.key == K_RIGHT and pointer.selected != 6:
                            pointer.update_position(pointer.selected+1)
                        if event.key == K_RETURN:
                            ...
                            if args.count_states == True:
                                ...
                                unique_state_list = []
                                print(
                                    f"Possible combinations: {num_combinations}, Duplicate combinations: {duplicate_combinations}")
                            if parser.track_win_progress == True:
                                check_n_in_row(slots, 1)
                                check_n_in_row(slots, 2)
                            turn_played = True
                    if event.key == K_SPACE and game_ended:
        ...
        if args.visualise == True:
            draw.rect(screen, (57, 137, 163, 255), BOARD_SIZE)
            for i in range(len(slots)):
                for n in range(len(slots[i])):
                    circle = slots[i][n]
                    draw.circle(circle.surface, circle.colour,
                ...
                if args.visualise == True:
                    if win_status == 2:
                        text = end_game_font.render(
                            "Yellow Wins!", True, Color(222, 205, 58), None)
                        # print("Yellow wins!")
                    elif win_status == 1:
                        text = end_game_font.render(
                            "Red Wins!", True, Color(235, 64, 52), None)
                        # print("Red wins!")
                    elif win_status == -1:
                        text = end_game_font.render(
                            "Tie", True, Color(128, 128, 128), None)
        ...
        if args.visualise == True:
            text = score_font.render(
                str(scores[1]), True, Color(222, 205, 58), None)
            textRect = text.get_rect()
            ...
            text = score_font.render(percent_string, True,
                                     Color(222, 205, 58), None)
            textRect = text.get_rect()
            textRect.center = (len(percent_string)*15, 50)
            screen.blit(text, textRect)

            text = score_font.render(
                str(scores[0]), True, Color(235, 64, 52), None)
            textRect = text.get_rect()
...
if args.visualise == False and args.mode == "RvR" and args.count_states == True:
    running = True
    while running:
        """
        A more optimised version of the main loop for if the program is solely being
        used for finding the number of possible board combinations.
        """
        turn_played = False
        if not game_ended:
             rand_col = int(random()*6)
